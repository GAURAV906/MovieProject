...
import TransitionModal from '../TransistionModal/TransistionModal';
...
class MovieContainer extends Component {
	...
	async getData(value, method) {
		try {
			...
			axios.get('http://127.0.0.1:8000/movies/?page=' + value).then(res => {
	...
	async onDeleteHandler(item) {
		...
		const res = await fetch(
			'http://127.0.0.1:8000/movies/' + item + '/delete/',
			{ method: 'DELETE' }
		);
		const movies = await res.json();
			...
			<div>
				{this.state.network ? (
					<div>
								...
								<div style={style.div} key={item.id}>
									...
									>
										<ListItem alignItems='flex-start'>
											...
											<TransitionModal key={item.id} itemId={item}/>
						...
						<div>
							<Pages
								handleOnChange={onPageChangeHandler}
					...
					<div>
						<Paper variant='elevation' elevation={3} style={style.noNetwork}>
							Make sure the backend server is on
						</Paper>
					</div>
				)}
			</div>
